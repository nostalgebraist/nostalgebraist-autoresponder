...
def make_session(reset=True):
  ...
  with sess.as_default():
      ...
      sample_pasts = tf.placeholder(tf.float32, model.past_shape(hparams=hparams, batch_size=batch_size))
...
def get_prompted_continuation(prompt: str,
    ...
    raw_text = final_munge_before_neural(prompt,
    ...
    while not done:
      ...
      sample_output_dict['presents'] = sample_output_dict['presents'][..., -(max_context_size-1):, :]
      ...
      if mirosurprises is None or (this_batch_continue_steps == first_step_with_miro):
          ...
          mirosurprises = np.concatenate([mirosurprises, sample_output_dict['mirostat_surprises']], axis=1)
      ...
      if continue_if_cut_off:
        ...
        batch_context_tokens = [np.concatenate(ct)[-(max_context_size):] for ct in continuations_tokens]
        ...
        while not all([bctl == token_start_ix for bctl in bct_lens]):
            print(f"weirdness: not all elements of batch_context_tokens have same length")
            for subtexts, nep, bct in zip(continuations, next_prompts, batch_context_tokens):
                st_lens = [len(enc.encode(st)) for st in subtexts]
                full_len = len(enc.encode("".join(subtexts)))
                nep_len = len(enc.encode(nep))
                bct_len = len(bct)
                print(f"st_lens={st_lens} | full_len={full_len} | nep_len={nep_len} | bct_len={bct_len}")
            batch_context_tokens = [bct[:token_start_ix] for bct in batch_context_tokens]
            bct_lens = [len(bct) for bct in batch_context_tokens]
            token_start_ix = min(bct_lens)
        ...
        not_finished = [c
                        for c, is_nf in zip(next_prompts_contonly, is_not_finished)
                        if is_nf]
        n_not_finished = len(not_finished)
        more_needed =  n_not_finished > 0
        more_permitted = (this_batch_continue_steps < max_continue_steps) and (tokens_generated < max_continue_tokens)

        show_miro_logs = MIRO and ((not MIRO_ONLY_ON_CONTINUE) or this_batch_continue_steps >= first_step_with_miro)
        ...
        if show_miro_logs:
          for i in range(batch_size):
            ...
            print(f'{finished_mark} {i}: targeting surprise {mirotarg:.3f}, avg surprise {miro_avg_surprises[i]:.3f}, median k {miro_median_ks[i]:.1f}, mean k {miro_mean_ks[i]:.1f}')

            if this_batch_continue_steps == first_step_with_miro:
                print([(j, enc.decode([tok]), mk, f"{ms:.3f}", f"{mmu:.3f}")
                       for j, (tok, mk, ms, mmu) in enumerate(zip(
                           out[i],
                           miroks[i, 1:],
                           mirosurprises[i].tolist()[1:],
                           sample_output_dict['mirostat_mus'][i].tolist()[1:],
                           ))
                       ])
            if i == batch_size-1:
                print()
        ...
        if not done:
          ...
          print(f"\t{tokens_generated}/{max_continue_tokens} continue tokens generated")
          print(f"\tcontext tokens sizes: {[len(ct) for ct in batch_context_tokens]}")
...
def get_prompt_from_dataset(dataset):
    ...
    if EOT_WORKAROUND:
      if EOT_PREPEND:
        ...
        n_extra_tokens += 1
      context_tokens = enc.encode(segment)[:len(enc.encode(look_for))+n_extra_tokens]
...
profane_substrings = {
  "shit", "fuck", "sex", "crap", "hell", "damn", "vagina", "penis",
   "genital", "piss", "gay",
   }
def basic_n_continuations(prompt, N,
  ...
  while len(continuations) < N:
    ...
    for c in this_batch_continuations:
      ...
      elif (len(c.partition("\n")[2].split(" ")) < avoid_half_if_under) and roll < 0.5:
      ...
      elif any([subs in c.lower() for subs in profane_substrings]) and avoid_if_profane:
        print(f"rejecting because profane: \n{fill(c)}\n")
      elif normalize_for_generator(c.partition(T_CHAR)[0].strip(whitespace)) in normalize_for_generator(prompt):
        print(f"rejecting because repeating myself: \n{fill(c)}\n")
      else:
        if (len(c.partition("\n")[2].split(" ")) < avoid_half_if_under):
          print(f"keeping with roll {roll}, although length under {avoid_half_if_under}")
...
def show_note_probas(texts, probas, sentiment_logit_diffs=None, console_width=110):
    if sentiment_logit_diffs is None:
        ...
        for tpe, proba, ld in zip(texts, probas, sentiment_logit_diffs):
            print(f"\tpredicted prob: {proba:.1%}, sentiment_logit_diff {ld:.4f}, pos_sent {logit_diff_to_pos_sent(ld):.1%}\n")
            print("\n~_~_~_~_~_\n")
            print("\n".join(wrap(tpe, replace_whitespace=False, width=console_width)))
            print("\n~_~_~_~_~_\n")
...
def model_activations(hparams, X, hparams_select,
  ...
  with tf.variable_scope(scope, reuse=reuse, dtype=dtype):
      ...
      for layer, past in enumerate(pasts):
          h, present, present_adapt = block(h, 'h%d' % layer, past=past, past_adapt=past, hparams=hparams)
...
def norm(x, scope, *, axis=-1, epsilon=1e-5, hparams=None):
  if hparams.get("scalenorm", False):
    ...
    with tf.variable_scope(scope, dtype=dtype):
      ...
      g = get_variable('g') or tf.get_variable('g', [1], initializer=tf.constant_initializer(1, dtype=dtype))
...
def mlp_acti_dropout(x, scope, n_state, *, hparams, w_init_stdev=1, n_final=None, dropout_final=True):
    dtype = hparams.dtype if hparams else tf.float32
    with tf.variable_scope(scope, dtype=dtype):
        nx = x.shape[-1].value
        if n_final is None:
...
def selector(hparams, X, hparams_select,
            layer_nums: list,
            scope="model", select_scope="select",
            reuse=tf.AUTO_REUSE,
            norm_layers_after: bool=False,
            use_mlp: bool=True,
            resid_mlp: bool=True,
  ...
  if add_prompt_cont_embs:
    with tf.variable_scope(select_scope, reuse=reuse):
      ...
      prompt_cont_embs = tf.get_variable('select_prompt_cont_embs', [2, hparams.n_embd],
      ...
      is_cont = tf.where(positions > tiled_prompt_end_ntoks, tf.ones_like(positions), tf.zeros_like(positions), )
  ...
  with tf.variable_scope(select_scope, reuse=reuse, dtype=hparams_select.dtype):
    for act_name, act in activations:
      ...
      selection_ix_position = tf.cast(tf.reshape(extract_selection_ix_position(X)['extracted'], [-1, 1]),
                                  tf.float32)
    ...
    if use_mlp:
      ...
      if mlp_proj:
        ...
        m = mlp_acti_dropout(h_select_in_at_selection_ix, "select_mlp", int(mlp_ratio*nx), hparams=hparams_select,
                             n_final=n_final, dropout_final=(not direct_mlp))
      else:
        m = mlp_no_proj(h_select_in_at_selection_ix, "select_mlp", nx, hparams=hparams_select)
      if direct_mlp:
        pass
      elif resid_mlp:
    ...
    if w_select is None:
      ...
      w_select = tf.get_variable('w_select', [len(layer_nums)*hparams.n_embd, 2],
    ...
    if use_length_channel:
      if use_length_channel_v2:
        ...
        if w_select_length_linear is None:
          w_select_length_linear = tf.get_variable('w_select_length_linear', [1, 2],
                                            initializer=tf.constant_initializer(0., dtype=hparams.dtype))

        w_select_length_log = get_variable('w_select_length_log')
        if w_select_length_log is None:
          w_select_length_log = tf.get_variable('w_select_length_log', [1, 2],
                                            initializer=tf.constant_initializer(0., dtype=hparams.dtype))

        select_logits = select_logits + tf.matmul(selection_ix_position, w_select_length_linear)
        select_logits = select_logits + tf.matmul(tf.log(selection_ix_position), w_select_length_log)

      else:
        w_select_length = get_variable('w_select_length')
        if w_select_length is None:
          w_select_length = tf.get_variable('w_select_length', [1, 2],
                                            initializer=tf.constant_initializer(0., dtype=hparams.dtype))
        select_logits = select_logits * (1 + tf.matmul(1+tf.log(selection_ix_position), w_select_length))
...
if SELECT_VIA_GENERATOR:
  with sess.as_default():
    ...
    prompt_end_ntoks = tf.placeholder(tf.int32, [batch_size_for_h], name="select_prompt_end_ntoks")

    selection_step = selector(select_scope=select_scope,
    hparams=hparams, hparams_select=hparams_select,
      X=context_for_h, layer_nums=layer_nums,
      norm_layers_after=norm_layers_after,
      use_mlp=use_mlp, resid_mlp=resid_mlp,
      direct_mlp=direct_mlp,
      mlp_proj=mlp_proj, mlp_ratio=mlp_ratio,
...
if SENTIMENT_VIA_GENERATOR:
  with sess.as_default():
    selection_step_sentiment = selector(
      ...
      norm_layers_after=norm_layers_after,
      use_mlp=use_mlp_sentiment, resid_mlp=resid_mlp, mlp_proj=mlp_proj,
...
if SELECT_VIA_GENERATOR:
  ...
  def single_batch_predict_select(data_batch, lr_calib, threshold=0.5, debug=False, truncate_at_right=TRUNCATE_AT_RIGHT,
                                  length_=length_select, override_disable_forumlike=False):
    ...
    for text in data_batch.selector_inputs:
      ...
      if debug:
        print(f"in single_batch_predict_select, predicting on:\n{enc.decode(batch_context[-1])}\n")
  ...
  def predict_select(data, lr_calib, threshold=0.5, debug=False, length_=length_select, override_disable_forumlike=False):
    batches = []
    ...
    for i in range(0, len(data), batch_size_for_h):
      ...
      if n_needed < batch_size_for_h:
          data_batch = pd.concat([data_batch] + (batch_size_for_h-n_needed)*[data_batch.iloc[-1:, :]],
                                  ignore_index=True)
      # while len(batch) != batch_size_for_h:
      #   batch = batch + [batch[-1] for _ in range(batch_size_for_h - len(batch))]
      batches.append(data_batch)

    batch_results = [single_batch_predict_select(data_batch, lr_calib, threshold=threshold, debug=debug, length_=length_, override_disable_forumlike=override_disable_forumlike)
     for data_batch in batches]

    result_keys = batch_results[0].keys()
    results = {k: np.concatenate([br[k] for br in batch_results])[:len(data)]
              for k in result_keys}

    return results

if SENTIMENT_VIA_GENERATOR:
  def single_batch_predict_sentiment(text_batch, length_=length_sentiment, debug=False):
    ...
    with sess.as_default():
      logits = sess.run(sentiment_logits, feed_dict={context_for_h.name: batch_context})
  ...
  def predict_sentiment(data, length_=length_sentiment, debug=False):
    ...
    batch_results = [single_batch_predict_sentiment(batch, length_=length_, debug=debug) for batch in batches]
  ...
  if kwargs.get("V5"):
    ...
    if SELECT_VIA_GENERATOR:
      if SELECTOR_CAN_SEE_PROMPTS:
        ...
        if FORUMLIKE:
            prompt_forumlike = substitute_forumlike(normalize_for_generator(prompt),
                                                    shuffle=False,
                                                    infer_first=False,
                                                    left_strip_newline=SELECTOR_LEFT_STRIP_NEWLINE_IN_FORUMLIKE)
      ...
      if SENTIMENT_VIA_GENERATOR:
        ...
        parsed["sentiment_logit_diffs"] = [float(p) for p in sentiment_results["logit_diffs"]]
        show_note_probas(continuations, probas=parsed["selection_proba"], sentiment_logit_diffs=parsed['sentiment_logit_diffs'])
      else:
...
def serve_textpost(data):
  ...
  if kwargs.get("V5"):
    try:
      continuations, continuation_side_data = basic_n_continuations(prompt,
                                            N=kwargs['best_of'],
                                            avoid_if_under=avoid_if_under,
                                            avoid_half_if_under=avoid_half_if_under,
                                            avoid_if_cut_off=avoid_if_cut_off,
...
def serve_raw_select(data):
  ...
  if SELECT_VIA_GENERATOR:
    ...
    if SENTIMENT_VIA_GENERATOR:
        selector_inputs = pd.DataFrame({"selector_inputs": [final_munge_after_neural(s) for s in texts]})
        sentiment_results = predict_sentiment(selector_inputs, debug=True)
        results["sentiment_logit_diffs"] = [float(p) for p in sentiment_results["logit_diffs"]]
        show_note_probas(texts, probas=results["selection_proba"], sentiment_logit_diffs=results['sentiment_logit_diffs'])
    else:
...
def poll(capture_ident=None, dummy=False):
  with capture_output(True, True, False) as cap:
    ...
    r = requests.post("{BRIDGE_SERVICE_REMOTE_URL}/pollgenerator", json={"results": RESULT_STACK if not dummy else {}})
    ...
    if len(PROMPT_STACK) > 0:
      ...
      if capture_ident is not None:
        ...
        with open(os.path.join(drivedir, capture_stdout_fn), "a", encoding="utf-8") as f:
          f.write(cap.stdout)
        if len(cap.stderr) > 0:
          capture_stderr_fn = f"AR_logs/{capture_ident}_stderr.txt"
          with open(os.path.join(drivedir, capture_stderr_fn), "a", encoding="utf-8") as f:
            f.write(cap.stderr)
  if capture_ident is not None:
