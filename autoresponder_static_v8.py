...
def get_ordered_interlocutors(doc, control_seg_config=DEFAULT_CSC):
    cchars = find_control_chars_forumlike(doc, incl_number=False, control_seg_config=control_seg_config)
...
def format_segment_v8_interlocutors(doc, control_seg_config=DEFAULT_CSC):
    names, n_names = get_ordered_interlocutors(doc, control_seg_config=control_seg_config)
    if len(names) == 0:
        return ""
    if len(names) == 1:
        name = names[0]
    ...
    return control_seg_config["conversation_between"].format(comma_names=comma_names, n_names=n_names)
...
def format_segment_v8_tags(tag_string_raw, user_name="Frank", control_seg_config=DEFAULT_CSC):
    if len(tag_string_raw)==0:
        ftags = ""
    else:
        ftags = ", ".join(["#"+t.rstrip(" ") for t in tag_string_raw.split("#")[1:]])
    return control_seg_config['user_tagged_post'].format(user_name=user_name, ftags=ftags)
...
def globally_format_v8(doc_tagless, ts_string, interlocutor_string, tag_string,
    ...
    lcc, lcc_loc = last_control_char(doc_tagless, extra_names=extra_names, control_seg_config=control_seg_config)
...
def final_munge_before_neural_v8(doc,
    ...
    normal_text = final_munge_before_neural_v7(normal_text, override_disable_forumlike=override_disable_forumlike, left_strip_newline=True, control_seg_config=control_seg_config, user_name=user_name, mode=mode)
    if override_disable_forumlike:
        return normal_text

    ts_string = format_segment_v8_time(time_text, control_seg_config=control_seg_config)

    interlocutor_string = format_segment_v8_interlocutors(normal_text, control_seg_config=control_seg_config)
    ...
    if forced_tags_string is not None and (forced_tags_string != ""):
        ...
        tag_string = format_segment_v8_tags(tag_string_raw, control_seg_config=control_seg_config, user_name=user_name)
...
def final_munge_before_neural_v10(doc, **kwargs):
    return final_munge_before_neural_v8(doc, control_seg_config=CONTROL_SEG_CONFIGS["V10"], **kwargs)
...
def final_munge_after_neural_v10(text):
  ...
  if ORIG_FICTION_CHAR_FORUMLIKE_V10 in text or REVIEW_CHAR_FORUMLIKE_V10 in text:
      expect_tag_prefix = False
  for cchar in [ORIG_POST_CHAR_FORUMLIKE_V10, REVIEW_CHAR_FORUMLIKE_V10, ORIG_FICTION_CHAR_FORUMLIKE_V10]:
    text = text.replace(cchar, "")
