...
def default_hparams():
    return HParams(
        ...
        acti_dropout_adapt=0.,
        attn_dropout_adapt=0.,
        res_dropout_adapt=0.,
        n_adapt_attn=128,
        nhead_adapt_attn=4,
        n_adapt_mlp=256,
...
def get_initializer(hparams, scope, fan_in, gain=None):
  ...
  if hparams.get("orth_init"):
    initializer = lambda **kwargs: tf.compat.v1.orthogonal_initializer(gain=gain, **kwargs)
  return initializer
...
def conv1d(x, scope, nf, *, gain=None, hparams=None):
    ...
    with tf.variable_scope(scope, dtype=dtype):
        *start, nx = shape_list(x)
        ...
        w = get_variable('w') or tf.get_variable('w', [1, nx, nf], initializer=initializer(dtype=dtype))
...
def attn(x, scope, n_state, *, past, hparams, n_head=None, gain=None, adapt=False):
    ...
    def multihead_attn(q, k, v):
        ...
        attn_dropout = hparams.get("attn_dropout_adapt", 0) if adapt else hparams.get("attn_dropout", 0)
    ...
    with tf.variable_scope(scope, dtype=dtype):
        ...
        res_dropout = hparams.get("res_dropout_adapt", 0) if adapt else hparams.get("res_dropout", 0)
...
def mlp(x, scope, n_state, *, hparams, gain=None, adapt=False):
    ...
    with tf.variable_scope(scope, dtype=dtype):
        ...
        h = gelu(conv1d(x, 'c_fc', n_state,
                        gain=gain,
                        hparams=hparams))
        acti_dropout = hparams.get("acti_dropout_adapt", 0) if adapt else hparams.get("acti_dropout", 0)
...
def block(x, scope, *, past, past_adapt, hparams, adapt=False, h_next=None, mix_next_weight_next=1.):
    dtype = hparams.dtype if hparams else tf.float32
    with tf.variable_scope(scope, dtype=dtype):
        nx = x.shape[-1].value
    ...
    if adapt and hparams.attn_adapters:
        ...
        with tf.variable_scope(adapter_scope):
            if hparams.adapters_ln_after_downproj:
                proj_down_fn = lambda _: norm(conv1d(_, f'proj_down_attn', hparams.n_adapt_attn, hparams=hparams), 'adapt_ln_1_attn', hparams=hparams)
                adapt_in = x
            else:
                proj_down_fn = lambda _: conv1d(_, f'proj_down_attn', hparams.n_adapt_attn, hparams=hparams)
                adapt_in = x_ln1
            proj_up_fn = lambda _: conv1d(_, 'proj_up_attn', nx, hparams=hparams)
            attn_fn = lambda _: attn(_, 'attn', hparams.n_adapt_attn, past=past_adapt, hparams=hparams, n_head=hparams.nhead_adapt_attn, adapt=True)
    ...
    if adapt and hparams.mlp_adapters:
        with tf.variable_scope(adapter_scope):
            if hparams.adapters_ln_after_downproj:
                proj_down_mlp_fn = lambda _: norm(conv1d(_, 'proj_down_mlp', hparams.n_adapt_mlp, hparams=hparams), 'adapt_ln_1_mlp', hparams=hparams)
                adapt_in = x
            else:
                proj_down_mlp_fn = lambda _: conv1d(_, 'proj_down_mlp', hparams.n_adapt_mlp, hparams=hparams)
                adapt_in = x_ln2
            ...
            if hparams.adapters_share_proj:
                ...
                m_adapt_down = mlp_fn(proj_down)  # this creates no variables so doesn't need a scope context manager
...
def past_shape_adapt(*, hparams, batch_size=None, sequence=None):
    return [batch_size, hparams.n_layer, 2, hparams.nhead_adapt_attn, sequence, hparams.n_adapt_attn // hparams.nhead_adapt_attn]
...
def model(hparams, X, past=None, past_adapt=None, scope='model', reuse=tf.AUTO_REUSE,
 midpoint=None, midpoint_vect=None,
 stop_grad_at_midpoint=False,
 stop_at_h_out=False, h_out_vect=None, silent=False):
    if not silent:
    ...
    with tf.variable_scope(scope, reuse=reuse, dtype=dtype):
        ...
        pasts_adapt = tf.unstack(past_adapt, axis=1) if past_adapt is not None else [None] * hparams.n_layer
        ...
        for layer, (past, past_adapt) in enumerate(zip(pasts, pasts_adapt)):
            ...
            h, present, present_adapt = block(h, 'h%d' % layer, past=past, past_adapt=past_adapt, hparams=hparams, adapt=adapt_here)
        ...
        if h_out_vect is not None:
            return tf.reduce_mean(tf.einsum('aij,aij->ai', results['h_out'], h_out_vect))
        if stop_at_h_out:
            return results
