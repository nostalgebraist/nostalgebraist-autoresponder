...
def sgdr_decay_with_warmup(learning_rate, global_step, warmup_steps, initial_period_steps,
               t_mul=1.0, m_mul=1.0, learning_rate_min=0.0, name=None):
  return tf.maximum(learning_rate_min, 
    tf.cond(global_step < warmup_steps,
        lambda: tf.cast(global_step,tf.float32)*learning_rate/warmup_steps,
        lambda: sgdr_decay(learning_rate, global_step, initial_period_steps=initial_period_steps, t_mul=t_mul, m_mul=m_mul)))
