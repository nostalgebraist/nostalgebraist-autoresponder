...
def load_logit_diff_sample():
    with open("logit_diff_sample.pkl.gz", "rb") as f:
        logit_diff_sample = pickle.load(f)
    return pd.Series(logit_diff_sample)

def estimate_expected_rejections(min_logit_diff: float, max_logit_diff: float, logit_diff_sample_series: pd.Series):
    return ((logit_diff_sample_series < min_logit_diff) | (logit_diff_sample_series > max_logit_diff)).mean()

def get_mood_by_name(mood_name: str):
    bound_names = {"no_lower_bound": 0.,
                   "no_upper_bound": 1.}
    ...
    for k, v in moods_original_flavor.items():
        ...
        moods_logit_diff_version[k]["min_allowed_score"] = pos_sent_to_logit_diff(moods_logit_diff_version[k]["min_allowed_score"])
        moods_logit_diff_version[k]["max_allowed_score"] = pos_sent_to_logit_diff(moods_logit_diff_version[k]["max_allowed_score"])
