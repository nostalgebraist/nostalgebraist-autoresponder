...
DETERMINER_CENTER_UPDATES = {pd.Timestamp('2020-08-20 01:00:00'): -2.4,
                             pd.Timestamp('2020-08-25 14:00:00'): -2.,
                             ...
                             pd.Timestamp('2020-12-10 23:45:00'): -2.,
                             pd.Timestamp('2020-12-18 15:35:00'): -2.2,
                             pd.Timestamp('2020-12-21 15:25:00'): -2.3,
                             WEIGHTED_AVG_START_TIME: 0.,
                             pd.Timestamp('2020-01-20 18:20:00'): 0.2,
                             pd.Timestamp('2020-01-29 08:35:00'): 0.1,
                             pd.Timestamp('2020-01-30 14:25:00'): 0.,
                             }
DETERMINER_MULTIPLIER_UPDATES = {pd.Timestamp('2020-08-25 17:00:00'): 0.1/RESPONSE_SCALE_BASE,
...
class DynamicMoodSystem:
    ...
    def determiner_center_series(self, determiner: pd.Series) -> pd.Series:
        determiner_center_s = pd.Series([self.determiner_center for _ in determiner.index],
                                        index=determiner.index)
    ...
    def determiner_multiplier_series(self, determiner: pd.Series) -> pd.Series:
        determiner_multiplier_s = pd.Series([1. for _ in determiner.index],
                                            index=determiner.index)
        for time in sorted(self.determiner_multiplier_updates.keys()):
            determiner_multiplier_s.loc[time:] = self.determiner_multiplier_updates[time]

        return determiner_multiplier_s


def compute_dynamic_mood_inputs(response_cache: ResponseCache,
    ...
    def _compute_determiner(row):
        if np.isfinite(row.p75_generated_logit_diff) and (row.generated_ts == row.generated_ts):
            if row.using_weighted_avg:
                ...
                return convert_p75_generated_logit_diff_to_user_input_logit_diff(row.p75_generated_logit_diff)
        else:
    ...
    duplicate_bug_filter = duplicate_bug_filter | (mood_inputs.index < DUPLICATES_BUGFIX_START_TS)
...
def compute_dynamic_mood_over_interval(mood_inputs: pd.DataFrame,
    ...
    sentiment_centered = system.determiner_multiplier_series(sentiment_centered) * sentiment_centered
...
def create_mood_graph(response_cache: ResponseCache,
    ...
    mood_inputs = compute_dynamic_mood_inputs(response_cache)
    lti_series = compute_dynamic_mood_over_interval(mood_inputs, start_time-pd.Timedelta(days=window_length_days), end_time, system).apply(ytrans)
...
def counterfactual_mood_graph(mood_inputs,
                              determiner_centers,
                              ...
                              end_time: datetime=None,
                              window_length_days: float=WINDOW_LENGTH_DAYS,
                              in_logit_diff_space: bool=True,) -> str:
    ytrans = pos_sent_to_logit_diff if in_logit_diff_space else lambda x: x
    if end_time is None:
        end_time = datetime.now()
    if start_time is None:
    ...
    for dc in determiner_centers:
        if dc is not None:
            new_dc_updates = {k: v for k, v in DETERMINER_CENTER_UPDATES.items() if k < left_time}
            new_dc_updates[left_time] = dc
            systems[f"dc={dc:.2f}"] = DynamicMoodSystem(determiner_center_updates=new_dc_updates)

        if determiner_multipliers is not None:
            for dm in determiner_multipliers:
                new_dm_updates = {k: v for k, v in DETERMINER_MULTIPLIER_UPDATES.items() if k < left_time}
                ...
                if f"dm={dm_s}" not in systems:
                    systems[f"dm={dm_s}"] = DynamicMoodSystem(determiner_multiplier_updates=new_dm_updates)

    lti_serieses = {}
    for name, system in tqdm(systems.items()):
        lti_series = compute_dynamic_mood_over_interval(mood_inputs, left_time, end_time, system).apply(ytrans)
        lti_series = lti_series.loc[start_time:end_time]
    ...
    for k in ["only_happy", "only_non_sad", "only_non_happy", "only_sad"]:
        plt.axhline(ytrans(MOOD_NAME_TO_DYNAMIC_MOOD_VALUE_MAP[k]),
                    # label=display_names[k],
                    ls='--', c=colors[k])

    golives = {pd.Timestamp('2020-11-22'): 'v9_golive_approx',
