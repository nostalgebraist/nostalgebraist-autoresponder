"""
TODO: remove this file and replace with current reward-scraping pipeline
...
def process_body(post):
    ...
    if len(tree_html) > 0:
        processed = (
            processed + UNAME_CHAR + "nostalgebraist-autoresponder" + A_CHAR + comment
        )
...
def get_prompt_and_continuation(post):
    ...
    continuation += (
        T_CHAR + " ".join([f"#{tag}" for tag in post.get("tags", [])]) + "<|"
    )
...
def full_notes(response_cache, post):
    ...
    notes = response_cache.query(
        CachedResponseType.NOTES, post_identifier, expected_notes=post["note_count"]
    )
    return notes
...
def notes_since_last_reblog(response_cache, post, verbose=False):
    ...
    this_ix = [
        ix
        for ix, n in enumerate(notes_ascending)
        if (n.get("post_id") == post_id) or n["type"] == "posted"
    ][-1]
    other_reblog_ixs = [
        ix
        for ix, n in enumerate(notes_ascending)
        if ix > this_ix and n["type"] == "reblog" and n["blog_name"] == blogName
    ]
...
def count_slots(ids_to_reward_data):
    ...
    n_unfilled_notes = len(
        [v for v in ids_to_reward_data.values() if v.get("note_count") is None]
    )
    n_unfilled_pc = len(
        [
            v
            for v in ids_to_reward_data.values()
            if (v.get("prompt") is None) or (v.get("continuation") is None)
        ]
    )
...
def show_slots(ids_to_reward_data):
    ...
    print(
        f"{n_unfilled_pc}/{n_total} need text, {n_unfilled_notes}/{n_total} need notes"
    )
...
def scrape_note_counts(
    client,
    max_to_scrape=None,
    offset=0,
    stop_on_seen_id=False,
    ids_to_reward_data=dict(),
    ignore_reblogs_from_dash=True,
    notes_only=True,
    fill_only=True,
):
    ratelimit_client = RateLimitClient.from_tumblr_rest_client(
    ...
    def _on_new_post_batch(posts, ids_to_reward_data):
        for p in posts:
            ...
            if post_id in ids_to_reward_data:
                ...
                if (
                    existing_note_count != note_count
                    and existing_note_count is not None
                ):
                    print(
                        f"updating note_count from {existing_note_count} to {note_count} for {post_id}"
                    )
    ...
    def _reward_post_screener(posts, ids_to_reward_data, fill_only=True):
        ...
        if ignore_reblogs_from_dash:
            ...
            screened = [
                p for p in screened if p.get("source_title", blogName) == blogName
            ]
    ...
    # scraping routine
    count_check_requests_start = ratelimit_client.get_ratelimit_data()["day"][
        "remaining"
    ]
    limit_ = 50
    offset_ = offset
    posts = []
    next_ = response_cache.client.posts(
        blogName, limit=limit_, offset=offset_, notes_info=True
    )["posts"]
    seen_id_count = 0
    new_id_count = None
    ...
    while (
        (len(next_) != 0)
        and len(posts) < max_to_scrape
        and (seen_id_count != new_id_count or (not stop_on_seen_id))
    ):
        print("\n")
        posts.extend(next_)
        ...
        new_post_batch = _reward_post_screener(
            next_, ids_to_reward_data, fill_only=fill_only
        )
        ...
        if len(new_ids) > 0 and len(starting_ids) > 0:
            ...
            if (seen_id_count > 0) and stop_on_seen_id:
                print(
                    f"ids {new_ids.intersection(starting_ids)} ({seen_id_count} ids) in both new and starting sets"
                )

        rld = ratelimit_client.get_ratelimit_data()
        count_check_requests_end = rld["day"]["remaining"]
        count_check_requests_diff = (
            count_check_requests_start - count_check_requests_end
        )
...
def scrape_and_save(
    ...
    max_to_scrape=50,
    reset_offset=False,
    save_path="reward/reward.pkl.gz",
    ignore_reblogs_from_dash=True,
    notes_only=True,
    fill_only=True,
):
    ...
    ids_to_reward_data, offset = scrape_note_counts(
        client,
        max_to_scrape,
        offset,
        ids_to_reward_data=ids_to_reward_data,
        ignore_reblogs_from_dash=ignore_reblogs_from_dash,
...
def scrape_new_and_save(
    ...
    ids_to_reward_data, offset = scrape_note_counts(
        client,
        max_to_scrape=max_to_scrape,
        offset=offset,
        stop_on_seen_id=stop_on_seen_id,
        ids_to_reward_data=ids_to_reward_data,
        ignore_reblogs_from_dash=ignore_reblogs_from_dash,
        notes_only=notes_only,
        fill_only=fill_only,
