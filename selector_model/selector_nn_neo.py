...
class NostARHeadAttention(nn.Module, GPTNeoAttentionMixin):
    ...
    def __init__(
        ...
        self.v_proj = nn.Linear(self.embed_dim, self.proj_dim, bias=True)  # vs bias=False in GPTNeo -nost
        self.out_proj = nn.Linear(self.proj_dim, self.proj_dim, bias=True)

    def classic_init(self, gain: float = 1.):
        with torch.no_grad():
            qkv_weight = torch.empty(2 * self.embed_dim + self.proj_dim, self.embed_dim, requires_grad=False)
            nn.init.orthogonal_(qkv_weight, gain)

            q_weight, k_weight, v_weight = torch.split(
                qkv_weight,
                [self.embed_dim, self.embed_dim, self.proj_dim],
                dim=-2
            )
