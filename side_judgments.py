...
def munge_side_judgment_payload(response, extract_ix, use_allen_payload_schema=True):
    result = {"selection_proba": response["selection_proba"][extract_ix], "sentiment": None}
    if use_allen_payload_schema:  # stuff like reversed logits etc.
        if response['sentiment_logit_diffs'] is not None:
            allen_schema = logit_diff_to_allen_schema(response['sentiment_logit_diffs'][extract_ix])
            result["sentiment"] = {"raw": response['sentiment_logit_diffs'][extract_ix],
...
def logit_diff_to_allen_schema(logit_diff: float):
    ...
    prob = prob_pos if logit_diff > 0 else (1.-prob_pos)
...
class SideJudgmentCache:
        ...
        if text not in self.cache:
            ...
            if response['sentiment'] is None:
                ...
                if self.rtok is None:
                    from transformers.tokenization_roberta import RobertaTokenizer
                    self.rtok = RobertaTokenizer.from_pretrained("roberta-large")
                allen_response = predict_sentiment_legacy(text, rtok=self.rtok, sleep_time=sleep_time)
                if allen_response is not None:
                    response['sentiment'] = {"raw": allen_response, "allen_schema": allen_response,
                                             "obtaine_via": "allen"}

            if response['selection_proba'] is not None and response['sentiment'] is not None:
    ...
    def query_multi(self, texts: list, sleep_time: float=0.2, batch_size: int=4, verbose=True):
        ...
        batch_ixs = [fill_ixs[j:j+batch_size] for j in range(0, len(fill_ixs), batch_size)]
    ...
    @staticmethod
    def load(path: str="side_judgment_cache.pkl.gz", verbose=True) -> 'SideJudgmentCache':
        cache = None
