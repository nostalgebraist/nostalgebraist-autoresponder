...
if datetime(2020, 7, 13) < datetime.now(tz=TZ_PST) < datetime(2020, 7, 21):
...
def respond_to_reblogs_replies(
    ...
    for ri_ix, reblog_identifier in enumerate(identifiers):
        ...
        thread = TumblrThread.from_payload(post_payload)
        thread = add_empty_reblog(thread, blog_name=blogName, timestamp=datetime.now(tz=TZ_PST))
        ...
        if (
            ...
            if (
                ...
                if sent.get("generated_logit_diff") is not None:
                    ...
                    # TODO: DRY
                    sent["generated_ts"] = datetime.now(tz=TZ_PST)
    ...
    for post_payload in submissions[::-1]:
        if post_payload.get("summary", "") == FOLLOW_COMMAND:
            ...
            path = create_mood_graph(
                response_cache,
                start_time=datetime.now(tz=TZ_PST) - pd.Timedelta(days=MOOD_GRAPH_N_DAYS),
...
if __name__ == "__main__":
    ...
    # _pr_name = datetime.now(tz=TZ_PST).strftime("%Y-%m-%d-%H-%M-%S")
