...
def extract_loss(
            ...
            try:
                ...
                if infer_resets and len(timestamps) > 0:
                    ...
                    if timestamp - last_timestamp > pd.Timedelta(seconds=60 * 60):
                        print(
                            f"inferred reset from {timestamp}, {last_timestamp}, {timestamp - last_timestamp}"
                        )
    ...
    if clip_reset_lr:
        filt = (df.lr_intended < df.lr_intended.shift(1)) & (
            df.lr_intended < df.lr_intended.shift(-1)
        )
        df.loc[filt, "lr"] = None
...
def load_loss(
    path="loss_logs/autoresponder_v9_experimental_const_lr.txt",
    xscale=1,
    alpha=0.99,
    noise_alpha=0.99,
    noise_alpha_G=None,
    ...
    return extract_loss(
        ...
        noise_alpha_G=noise_alpha_G,
        noise_alpha_S=noise_alpha_S,
        infer_resets=infer_resets,
        clip_large_grads=clip_large_grads,
    )
...
def _compute_lr_scale(df, ref_col):
    ...
    print(
        f"typical_ref {typical_ref:.2f}, min_ref {min_ref:.2f}, max_ref {max_ref:.2f}"
    )
...
def show_noise_scale(
    ...
    with warnings.catch_warnings():
        ...
        axes[0].legend()
        to_plot.B_simple.plot(
            lw=0, markersize=ms, alpha=opacity, label="B_simple ewm", **kwargs
        )
        ...
        to_plot.avg_G_noise.plot(
            lw=0, markersize=ms, alpha=opacity, label="G_noise ewm", **kwargs_G
        )
        to_plot.avg_S_noise.plot(lw=1, markersize=0, alpha=opacity / 2, **kwargs_S)
        axes_1_right.legend(loc="lower left")
        to_plot.avg_S_noise.plot(
            lw=0, markersize=ms, alpha=opacity, label="S_noise ewm", **kwargs_S
        )
...
def noise_scale_load_and_show(
    ...
    axes = show_noise_scale(
        df,
        delay_show=True,
        opacity=opacity,
        loss_indexed=loss_indexed,
        loss_axmin=loss_axmin,
        step_axmin=step_axmin,
    )
    ...
    if showlr:
        for ax, col in zip(axes[:1], ["B_simple", "B_simple_inst"][:1]):
            ...
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", module="pandas")
                lr_series.plot(
                    ax=ax_lr, c=lrcolor, ls="--", alpha=max(0.2, 0.5 * opacity)
                )
...
def load_and_show(
    ...
    if show_val and "val_losses" in df.columns:
        df.val_losses.dropna().plot(
            marker=".", lw=1.5, ls="-", markersize=10, alpha=0.67 * opacity, c="k"
        )
...
def comparison_load_and_show(
    ...
    _, ax1, ax_lr = load_and_show(
        ...
        eplen=eplen_ref,
        epcolor="orange",
        sample_period=sample_period_ref,
        xscale=xscale_ref,
        delay_show=True,
        sets_ticks=True,
        alpha=alpha,
...
def val_show(
    ...
    if eplen is not None:
        eplens = [
            e
            for e in range(eplen, to_plot.index.max(), eplen)
            if e > to_plot.index.min()
        ]
...
def val_vs_train_show3(
    df, ax=None, delay_show=False, step_axmin=None, eplen=None, lrindexed=False
):
    _valdf = df[df.val_losses.notnull()]
    ...
    to_plot.val_losses.plot(
        marker=".", lw=1.5, ls="-", markersize=10, alpha=0.67, c="k", ax=ax
    )
    to_plot.avg_losses.plot(
        marker=".", lw=1.5, ls="-", markersize=10, alpha=0.67, ax=ax
    )
...
def val_load_and_show(
    ...
    with warnings.catch_warnings():
        ...
        val_vs_train_show3(
            df, ax=axes[1], step_axmin=step_axmin, eplen=eplen, lrindexed=lrindexed
        )
