...
def mt_annotate(df):
    ...
    df["pre_continue_length"] = df.model_info.apply(
        nancall(lambda d: d.get("sampling_info", {}).get("pre_continue_length", True))
    )
    df["ntok_miro"] = None
    df.loc[df.mtlen.notnull(), "ntok_miro"] = (
        df.loc[df.mtlen.notnull(), "mtlen"]
        - df.loc[df.mtlen.notnull(), "pre_continue_length"]
    )
    df["mtplotarg"] = [
        (mt, v2, pcl)
        for mt, v2, pcl in zip(
            df.miro_traces.values, df.is_miro_v2.values, df.pre_continue_length.values
        )
    ]
    return df
